...
2. **Resource owner**: 본인 계정, **Repository access**: 이 레포 1개만 선택
3. **Repository permissions** → `Contents` = **Read and write**
4. Expiration 은 1년으로 설정. (만료 1주일 전 이메일 알림이 옴)
5. 생성된 토큰 복사
6. 레포 → Settings → Secrets and variables → Actions → **New repository secret**
   - Name : `KEEPALIVE_PAT`
   - Secret: 위에서 복사한 토큰
7. Actions 탭 → "keepalive" 워크플로우 → "Run workflow" 로 즉시 1회 검증
